# Custom Restricted License (No-Copy, No-Redistribution)

Copyright (c) 2025 Ayush Raut

Permission is hereby granted to any individual or organization to:
- View the source code.
- Run the software locally.
- Modify the software for **personal, non-commercial use only**.

The following actions are **strictly prohibited** without explicit written permission:
- Copying or redistributing this codebase.
- Publishing any derivative works, modified versions, or replicas.
- Re-uploading, re-hosting, or mirroring the project.
- Using the code or machine learning models for commercial purposes.
- Training models using this dataset, code, or features.
- Integrating this project into any public or paid product.

This software is provided "as-is" without warranty of any kind.  
By using this repository, you agree to these terms.

For licensing inquiries, contact:  
**rautayush@gmail.com**
